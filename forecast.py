...
class WaterTemperatureModel:
    ...
    def seasonal_offset(self, day_of_year):
        ...
        return self.seasonal_amp * np.sin(2 * np.pi * (day_of_year - self.seasonal_phase) / 365)
    ...
    def fit_parameters(self, air_temps, observed_water_temps, dates):
        ...
        def objective(params):
            ...
            predicted = self.predict_temperature(air_temps, observed_water_temps[0], dates)
    ...
    def forecast(self, air_temp_forecast, current_water_temp, start_date, forecast_days=14):
        """
        ...
        return self.predict_temperature(air_temp_forecast[:forecast_days], 
...
def prepare_real_data():
    ...
    print(f"✅ Merged dataset: {len(merged_df)} points with both water and air temperature")
...
def generate_synthetic_data():
    ...
    air_temps = 12 + 8 * np.sin(2 * np.pi * (day_of_year - 80) / 365) + np.random.normal(0, 3, n_days)
    
    # "True" water temperatures (for testing) - more damped, delayed
    water_temps = 12 + 5 * np.sin(2 * np.pi * (day_of_year - 120) / 365) + np.random.normal(0, 1, n_days)
...
def demo_model(use_real_data=True):
    ...
    predicted_test = model.predict_temperature(test_air, predicted_train[-1], test_dates)
    ...
    plt.subplot(3, 1, 1)
    plt.plot(train_dates, train_air, 'b-', alpha=0.6, linewidth=1, label='Air Temperature')
    plt.plot(train_dates, train_water, 'ro', markersize=2, label='Observed Water Temp')
    plt.plot(train_dates, predicted_train, 'g-', linewidth=2, label='Predicted Water Temp')
    plt.title(f'Training Period - {data_source} Data (RMSE: {train_rmse:.2f}°C)', fontsize=14)
    plt.ylabel('Temperature (°C)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plt.subplot(3, 1, 2)
    plt.plot(test_dates, test_air, 'b-', alpha=0.6, linewidth=1, label='Air Temperature')
    plt.plot(test_dates, test_water, 'ro', markersize=2, label='Observed Water Temp')
    plt.plot(test_dates, predicted_test, 'g-', linewidth=2, label='Predicted Water Temp')
    plt.title(f'Testing Period - Model Validation (RMSE: {test_rmse:.2f}°C)', fontsize=14)
    plt.ylabel('Temperature (°C)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.plot([min_temp, max_temp], [min_temp, max_temp], 'r--', alpha=0.8, label='Perfect Prediction')
...
if __name__ == "__main__":
    ...
    if data_result[0] is not None:
        ...
        print(f"  📈 Temperature change: {forecast[-1] - current_water_temp:+.1f}°C over 14 days")
