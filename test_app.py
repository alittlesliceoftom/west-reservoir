...
class TestOutageWindowEnds:
    ...
    def test_no_band_once_the_window_has_scrolled_out_of_view(self):
        ...
        _shade_meteostat_outage(
            fig, pd.Timestamp("2026-09-20"), pd.Timestamp("2026-10-01"), horizon=1
        )
...
class TestDashboardModel:
    ...
    def test_a_failed_fit_is_not_cached(self):
        """
        Streamlit does not memoise exceptions, so a transient failure is retried
        rather than pinned for the TTL. That is what makes caching safe here.
        """
        measured, weather = self._measured(3), self._weather()
        dashboard_model(measured, weather)
        with patch.object(WaterTempForecaster, "fit", autospec=True) as fit:
            fit.return_value = False
            dashboard_model(measured, weather)
        fit.assert_called()
